...
<dsp:page>
	...
	<dsp:importbean
		bean="/atg/endeca/assembler/cartridge/StoreCartridgeTools"
		var="StoreCartridgeTools" />
...
<head>
	...
	<%-- Javascript on css style --%>
		document
				.write('<link rel="stylesheet" href="${contextPath}/css/javascript.css" type="text/css" charset="utf-8" />');
	...
	<c:set var="indexValue"
		value="${(index eq 'false') ? 'noindex' : 'index'}" />
	<c:set var="followValue"
		value="${(follow eq 'false') ? 'nofollow' : 'follow'}" />

	<meta name="robots" content="${indexValue},${followValue}" />

	<%-- Include content from SEO tag renderer --%>
	<dsp:getvalueof var="SEOTagRendererContent"
		param="SEOTagRendererContent" />
	<c:choose>
		...
		<c:otherwise>
			...
			<dsp:getvalueof var="currentCategoryId"
				value="${StoreCartridgeTools.currentCategoryId}" />
			<c:choose>
	...
	<dsp:getvalueof var="faviconUrl" vartype="java.lang.String"
		value="${currentSite.favicon}" />
